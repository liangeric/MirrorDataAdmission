...
# set seed
#seed = 12
def runModel(data1Path, matched2Path, seed, verbose = False):
    np.random.seed(seed)

    # Read in appropriate data
    data1 = pd.read_csv(data1Path)
    matchedData2 = pd.read_csv(matched2Path)
    ...
    def metric_report(train_data, predicted_labels, verbose = False):
        metrics = []
        # Metrics for Race
    ...
    #print("------------------------------")
    experiment_a = metric_report(idealX_valid,ideal_predictions,verbose)
    #print("------------------------------")
    experiment_b = metric_report(realX_valid,real_predictions, verbose)
    #print("------------------------------")
